...
namespace ResellerPanelAuthGG
{
                ...
                try
                {
                    ...
                    switch (response[2])
                    {
                        ...
                        case "binderror":
                            ...
                            return;
                        case "banned":
                            MessageBox.Show("This application has been banned for violating the TOS" + Environment.NewLine + "Contact us at cheatloop.com", Name);
                            Process.GetCurrentProcess().Kill();
                            return;
                    }
                    Security.End();
    ...
    internal class API
    {
        ...
        public static bool Register(string username1, string password1, string email1, string license1)
        {
            if (!Constants.Initialized)
            {
                MessageBox.Show("Please initialize your application first!", OnProgramStart.Name);
            ...
            if (string.IsNullOrWhiteSpace(username1) || string.IsNullOrWhiteSpace(password1) || string.IsNullOrWhiteSpace(email1) || string.IsNullOrWhiteSpace(license1))
            {
                MessageBox.Show("Invalid registrar information!", ApplicationSettings.Name);
                Process.GetCurrentProcess().Kill();
            }
            ...
            using (WebClient wc = new WebClient())
            {
                try
                {
                    ...
                    response = Encryption.DecryptService(Encoding.Default.GetString(wc.UploadValues(Constants.ApiUrl, new NameValueCollection
                    {
                        ...
                        ["secret"] = Encryption.APIService(OnProgramStart.Secret),
                        ["type"] = Encryption.APIService("register"),
                        ["username"] = Encryption.APIService(username1),
                        ["password"] = Encryption.APIService(password1),
                        ["email"] = Encryption.APIService(email1),
                        ["license"] = Encryption.APIService(license1),
                        ["hwid"] = Encryption.APIService(Constants.HWID()),
